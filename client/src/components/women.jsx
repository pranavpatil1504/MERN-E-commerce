// client/src/components/Women.jsx
...
import womens from './data/womendata';
...
const Women = () => {
    ...
    useEffect(() => {
        setAddedToCart(womens.map(product => cartItems.some(cartItem => cartItem.id === product.id)));
    ...
    const getUserEmail = () => {
        // Logic to retrieve the user's email, for example from localStorage
        const userEmail = localStorage.getItem('userEmail');
        return userEmail;
    };
    const handleAddToCart = async (product, index) => {
        // Retrieve the user's email using getUserEmail function
        ...
        try {
            // Send cart item details along with user's email to the backend
    ...
    const filteredProducts = womens.filter(product =>
